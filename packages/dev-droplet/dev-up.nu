...
def main [] {
  ...
  # Get SSH key fingerprint from DO (using the GitHub SSH Key)
  let ssh_fingerprint = "6f:9a:79:89:7d:ec:d4:86:02:68:92:d2:60:92:4b:be"

  cd $infra_dir

  # Initialize if needed
  if not ($"($infra_dir)/.terraform" | path exists) {
    print $"(ansi cyan)Initializing terraform...(ansi reset)"
    tofu init
  }

  # Apply with droplet enabled
  print $"(ansi cyan)Creating droplet...(ansi reset)"
  (tofu apply
    -var $"do_token=($do_token)"
    -var $"ssh_key_fingerprint=($ssh_fingerprint)"
    -var "droplet_enabled=true"
    -var "region=sfo3"
    -var "droplet_size=s-4vcpu-8gb"
    -auto-approve)

  print $"(ansi green)Droplet is up!(ansi reset)"
  print "Connect with: ssh dev"
}
